...
HTTP request header, 요청 url, cookies, query, body
등의 정보가 저장되어 있습니다.
*/
...
object. HTTP response header, cookies, HTTP code
등의 정보를 확인하고 값을 변경할 수도 있습니다.
*/
/*
...
/*
next
만약 여러개의 콜백 함수를 사용한다면 이 함수를 호출하여
다음번 콜백 함수로 넘어갈 수 있습니다.
...
/*
git bash에 $ node index.js 입력해서 서버를 실행
잘 되었다면 서버 실행 콘솔 로그가 뜨면서 서버가 실행됨
웹 브라우저를 열고 http://localhost:3000 에 접속하면
주소를 입력하는 것이 서버에 HTTP Method 중 GET으로 요청된다.
그러므로 app.get 콜백 함수가 실행되어 res.send를 통해
입력 텍스트가 웹 브라우저에 출력된다.
*/
